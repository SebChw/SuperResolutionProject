import numpy as np
import cv2
from torchsr.utils.utils import collect_paths
...
def save_patch(img, box, path, save_function):
    """Cuts box from image and saves it

    Args:
        img (np.ndarray): 
        box (list): 
        path (str): 
        save_function (): function that should be used to save image, depending on you want to have png or npy
    """
    x1, y1, x2, y2 = box
...
def cut_patches(config):
    """Iterates over all images and create patches of appropriate size

    Args:
        config (dict): yaml configuration, look in configs directory
    """
